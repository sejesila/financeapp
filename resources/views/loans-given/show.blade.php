                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <div class="flex items-center space-x-2">
                            @if($loanGiven->status === 'active')
                                ...
                                @if($isOverdue)
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800">Overdue</span>
                                @endif
                            @elseif($loanGiven->status === 'paid')
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">Paid</span>
                            @elseif($loanGiven->status === 'defaulted')
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800">Defaulted</span>
                            @elseif($loanGiven->status === 'written_off')
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-800">Written Off</span>
                            @endif
                        </div>
                    </div>
                                ...
                                <div class="flex justify-between py-2 border-b border-gray-100">
                                    ...
                                    <dd class="text-sm text-gray-900">
                                        @if($loanGiven->referrer)
                                            <a href="{{ route('referrers.show', $loanGiven->referrer->id) }}" class="text-indigo-600 hover:text-indigo-900">
                                    ...
                                    <div class="flex justify-between py-2 border-b border-gray-100">
                                        ...
                                        <dd class="text-sm text-gray-900">
                                            @if($loanGiven->referrer_deducted_before_deposit)
                                                KES {{ number_format($loanGiven->referrer_retained_amount, 0) }}
                                                <span class="text-xs text-gray-500">(already kept before depositing — nothing owed)</span>
                                            @elseif($referrerPayout !== null)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6 pt-6 border-t border-gray-200">
                        <div>
                            ...
                            <span
                                class="ml-2 text-sm text-gray-900">{{ $loanGiven->disbursed_date->format('M d, Y') }}</span>
                            @if($daysElapsed > 0)
                        ...
                        @if($loanGiven->due_date)
                            <div>
                                ...
                                <span
                                    class="ml-2 text-sm text-gray-900">{{ $loanGiven->due_date->format('M d, Y') }}</span>
                        ...
                        @if($loanGiven->repaid_date)
                            <div>
                                ...
                                <span
                                    class="ml-2 text-sm text-gray-900">{{ $loanGiven->repaid_date->format('M d, Y') }}</span>
                            </div>
                        @endif
                    </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                @foreach($loanGiven->payments as $payment)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                            KES {{ number_format($payment->amount, 0) }}</td>
                                ...
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-900 uppercase tracking-wider">
                                        KES {{ number_format($loanGiven->payments->sum('amount'), 0) }}</th>
                                    <th colspan="2"></th>
                                </tr>
                                </tfoot>
                            </table>
        ...
        <dialog id="closeLoanModal" class="rounded-lg shadow-xl w-full max-w-md">
            <div class="p-6">
                ...
                <form method="POST" action="{{ route('loans-given.close', $loanGiven->id) }}">
                    ...
                    @endif
                    @if($loanGiven->referrer)
                        <div class="mb-4 border border-purple-200 bg-purple-50 rounded-lg p-4">
