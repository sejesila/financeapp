...
@php
    ...
    // Net worth is now computed directly from the ledger (see
    // ReportDataService::generateReport()): pooled account cash + Outstanding
    // Loans Given (gross, no longer netted against anything) - Active Loans -
    // money borrowed from client funds and not yet returned. Outstanding
    // Loans Given is therefore shown GROSS everywhere below, including the
    // dedicated "Loans Given Activity" table further down, whose own "Total
    // Outstanding" row must match the sum of the per-borrower rows shown
    // directly under it anyway.
    $totalUnreturnedBorrowed = $data['total_unreturned_borrowed'] ?? 0;
...
<div class="net-worth-banner">
    ...
    <div class="breakdown">
        ...
        @if($totalUnreturnedBorrowed > 0)
            &bull; Borrowed (Unreturned): {{ $currency }} {{ number_format($totalUnreturnedBorrowed) }}
        @endif
    </div>
    @if($totalUnreturnedBorrowed > 0)
        <p style="font-size: 8px; color: rgba(255,255,255,0.85); margin-top: 4px;">
            Net worth above is reduced by {{ $currency }} {{ number_format($totalUnreturnedBorrowed) }} in client
            funds borrowed for personal use that has not yet been returned.
        </p>
    @endif
</div>
...
<table class="stat-table">
    ...
    <tr>
        ...
        <td class="value"
            style="color: {{ $netFlow >= 0 ? '#059669' : '#DC2626' }};">{{ $netFlow >= 0 ? '+' : '' }}{{ $currency }} {{ number_format($netFlow) }}</td>
        <td class="label">Savings Rate</td>
        <td class="value"
            style="color: {{ $savingsRate >= 20 ? '#059669' : ($savingsRate >= 10 ? '#D97706' : '#DC2626') }};">{{ number_format($savingsRate, 1) }}
            %
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td class="value"
            style="color: {{ $incomeTrend === null ? '#9CA3AF' : ($incomeTrend >= 0 ? '#059669' : '#DC2626') }};">
    ...
    <tr>
        ...
        <td class="value"
            style="color: {{ $profitMonths >= 10 ? '#059669' : ($profitMonths >= 6 ? '#D97706' : '#DC2626') }};">{{ $profitMonths }}
            /12
        </td>
        <td class="label">Loan Repayments</td>
        <td class="value"
            style="color: #059669;">{{ $loansPaid['count'] > 0 ? $currency . ' ' . number_format($loansPaid['total']) . ' (' . $loansPaid['count'] . ')' : 'None' }}</td>
    </tr>
    <tr>
        <td class="label">Annual Budget</td>
        <td class="value"
            style="color: {{ $annualBudgetedExpenses == 0 ? '#9CA3AF' : ($annualBudgetVariance >= 0 ? '#059669' : '#DC2626') }};">
        ...
        <td class="value"
            style="color: #059669;">{{ $loansCleared['count'] > 0 ? $loansCleared['count'] . ' (' . $currency . ' ' . number_format($loansCleared['principal_total']) . ' principal)' : 'None' }}</td>
    </tr>
    <tr>
        <td class="label">Best Month</td>
        <td class="value"
            style="color: #059669;">{{ $bestMonth ? $bestMonth['month'] . ' (+' . $currency . ' ' . number_format($bestMonth['net_flow']) . ')' : '—' }}</td>
        <td class="label">Toughest Month</td>
        <td class="value"
            style="color: #DC2626;">{{ $worstMonth ? $worstMonth['month'] . ' (' . $currency . ' ' . number_format($worstMonth['net_flow']) . ')' : '—' }}</td>
    </tr>
    <tr>
...
@if($savingsRate >= 20)
    <div class="alert success"><strong>Excellent annual financial health</strong>
        &mdash; {{ number_format($savingsRate, 1) }}% savings rate in {{ $year }}.
    </div>
@elseif($savingsRate >= 10)
    <div class="alert info"><strong>Good progress on savings</strong> &mdash; {{ number_format($savingsRate, 1) }}% this
        year. Aim to push above 20%.
    </div>
@else
    <div class="alert warning"><strong>Low savings rate</strong> &mdash; {{ number_format($savingsRate, 1) }}%, below
        the recommended 20%.
    </div>
@endif
    ...
    <div class="section">
        ...
        <table>
            ...
            <tr class="total-row">
                ...
                <td style="text-align: right; color: {{ $overallPct >= 20 ? '#059669' : '#D97706' }};">{{ $overallPct }}
                    %
                </td>
            </tr>
            </tbody>
...
@if($loansGiven['disbursed_count'] > 0 || $loansGiven['repayments_count'] > 0 || $activeLoansGiven->isNotEmpty())
    <div class="section">
        ...
        {{-- This table's "Total Outstanding" is the same GROSS sum of active
             ...
             matches the sum of the rows shown directly below it. --}}
        ...
        @if(!empty($loansGiven['items']))
            <div class="month-drill">
                <div class="month-drill-title">Loans Fully Repaid in {{ $year }}</div>
                <table>
                    <thead>
                    <tr>
                        <th>Borrower</th>
                        ...
                        <th style="text-align: center;">Repaid Date</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach($loansGiven['items'] as $item)
                        <tr>
                            <td style="font-weight: 600;">{{ $item['borrower'] }}</td>
                            <td style="text-align: right;">{{ $currency }} {{ number_format($item['principal']) }}</td>
                            <td style="text-align: right; color: #059669;">{{ $currency }} {{ number_format($item['interest']) }}</td>
                            <td style="text-align: center; color: #6B7280;">{{ $item['repaid_date'] }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
                    ...
                    <table>
                        ...
                        @foreach($month['category_performance'] as $budget)
                            ...
                            <tr>
                                ...
                                <td style="text-align: right; font-weight: bold; color: {{ $pctColor }};">{{ number_format($budget['percentage'], 1) }}
                                    %
                                </td>
                            </tr>
                        @endforeach
        ...
        <table>
            ...
            @foreach($data['accounts'] as $account)
                ...
                <tr>
                    ...
                    <td style="text-align: center;"><span class="badge {{ $healthClass }}">{{ $healthLabel }}</span>
                    </td>
...
<div class="footer">
    ...
    <p style="margin-top: 4px; font-size: 8px; color: #9CA3AF;">This document contains highly confidential financial
        information. Store securely and do not share with unauthorized parties.</p>
</div>
