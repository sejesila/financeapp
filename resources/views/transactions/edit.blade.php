    ...
    <div class="max-w-2xl mx-auto mt-4 sm:mt-10 p-4 sm:p-6 bg-white dark:bg-gray-800 shadow-xl rounded-xl sm:rounded-2xl">
        ...
        <form action="{{ route('transactions.update', $transaction) }}" method="POST" class="space-y-4 sm:space-y-5">
            ...
            <div x-data="transactionForm()">
                ...
                <!-- Category — identical HTML to create.blade.php, just different oldId -->
                <div class="mb-4 sm:mb-5">
                ...
                <div class="mb-4 sm:mb-5">
                    ...
                    <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">e.g. pay part in cash, part via M-Pesa</p>
                </div>
                ...
                <!-- SPLIT PAYMENT — identical HTML to create.blade.php -->
                <div x-show="isSplit" x-transition>
                    <input type="hidden" name="is_split" value="1" x-show="isSplit">
                    <div class="mb-3">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="text-xs sm:text-sm font-medium"
                                ...
                                <span x-text="formatAmount(splitTotal)"></span> / <span x-text="formatAmount(parseFloat(totalAmount) || 0)"></span>
                            ...
                            <template x-for="(split, index) in splits" :key="split.id">
                                <div class="border dark:border-gray-600 rounded-lg p-3 bg-gray-50 dark:bg-gray-700/50 space-y-2">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <button type="button" @click="removeSplit(index)" x-show="splits.length > 2"
                                    ...
                                    <select :name="'splits[' + index + '][account_id]'" x-model="split.account_id"
                                    ...
                                    <div x-show="split.showMobileType">
                                        <select :name="'splits[' + index + '][mobile_money_type]'" x-model="split.mobile_money_type"
                                                class="w-full text-sm border dark:border-gray-600 p-2 rounded focus:outline-none focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" step="0.01" :name="'splits[' + index + '][amount]'"
                                               x-model="split.amount" @input="onSplitAmountChange()" placeholder="0.00"
    ...
    <script>
        function transactionForm() {
            return {
                ...
                // Split state — pre-populate if editing a split transaction
                isSplit: {{ $transaction->is_split ? 'true' : 'false' }},
                splits: @json($existingSplits),
                splitIdCounter: {{ count($existingSplits) }},
                    ...
                    this.$nextTick(() => {
                        ...
                        this.splits.forEach((split, i) => {
                            if (split.showMobileType) {
                                const el = document.querySelectorAll('select[name^="splits["][name$="[account_id]"]')[i];
                                if (el) {
                                    const type = el.options[el.selectedIndex]?.getAttribute('data-type');
                                    split.typeOptions = type === 'mpesa' ? this.mpesaTypes : this.airtelTypes;
                                }
                            }
                        });
                    });
                ...
                addSplit() {
                    this.splits.push({ id: ++this.splitIdCounter, account_id: '', amount: '', mobile_money_type: 'send_money', showMobileType: false, typeOptions: [] });
                },
                ...
                onSplitAmountChange() {
                    ...
                    if (total > 0 && this.splits.length >= 2) {
                        ...
                        if (remainder >= 0) this.splits[this.splits.length - 1].amount = remainder;
                    }
                },
                ...
                onSplitAccountChange(index) {
                    ...
                    const accountType = selectEl.options[selectEl.selectedIndex].getAttribute('data-type');
                    if (accountType === 'mpesa') {
                        split.showMobileType = true; split.typeOptions = this.mpesaTypes; split.mobile_money_type = this.defaultMpesaType;
                    } else if (accountType === 'airtel_money') {
                        split.showMobileType = true; split.typeOptions = this.airtelTypes; split.mobile_money_type = this.defaultAirtelType;
                    } else {
                        split.showMobileType = false; split.typeOptions = []; split.mobile_money_type = '';
                    }
                },
        ...
        // categoryDropdown() function — identical to create.blade.php
