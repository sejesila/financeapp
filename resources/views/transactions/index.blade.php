        ...
        <div class="mx-auto max-w-7xl px-3 sm:px-6 space-y-6">
            ...
            @if(session('show_balance_modal'))
                <div id="balanceModal"
                    ...
                    <div
                        class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md mx-auto animate-fade-in">
                        <div class="p-6">
                            ...
                            <div class="mt-4 space-y-2 bg-gray-50 dark:bg-gray-700 p-3 rounded-xl">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="font-semibold text-sm text-gray-900 dark:text-white">KES {{ session('old_balance') }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Transaction:</span>
                                    <span
                                        class="font-semibold text-sm {{ session('transaction_type') === 'expense' ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400' }}">
                                ...
                                <div
                                    ...
                                    <span
                                        class="font-bold text-lg {{ floatval(str_replace(',', '', session('new_balance'))) < 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400' }}">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                    ...
                    @foreach($periodStats as $label => $data)
                        ...
                        <div
                            class="stat-card rounded-2xl shadow-lg p-4 text-white bg-gradient-to-br {{ $periodGradients[$label] ?? 'from-slate-500 to-slate-700' }}">
                            ...
                            <div class="border-t border-white/20 pt-2.5 flex justify-between items-center">
                                ...
                                <span
                                    class="text-sm font-bold tabular-nums {{ $net >= 0 ? 'text-emerald-300' : 'text-red-300' }}">
            ...
            <div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    ...
                    @foreach($feeCards as $card)
                        <div
                            class="stat-card rounded-2xl shadow-lg p-4 text-white bg-gradient-to-br {{ $card['gradient'] }}">
            ...
            <div
                ...
                <div class="flex items-center gap-2 pt-2 border-t dark:border-gray-700">
                    ...
                    <span
                        class="text-xs text-gray-500">({{ $showFees ? 'Currently showing fees' : 'Fees hidden' }})</span>
                </div>
                ...
                <details {{ in_array($filter, ['custom']) || $search || $categoryId || $accountId ? 'open' : '' }}>
                    ...
                    <form method="GET" action="{{ route('transactions.index') }}"
                        ...
                        <div class="col-span-full flex flex-wrap gap-2">
                            <button
                                class="rounded-lg bg-indigo-600 px-4 py-2 text-sm text-white hover:bg-indigo-700 shadow-md transition-all">
                                Apply
                            </button>
            ...
            {{-- Transactions Table --}}
            <div
                class="overflow-x-auto rounded-2xl border border-white/60 bg-white dark:bg-gray-800 dark:border-gray-700 shadow-lg">
                            ...
                            <th class="{{ $thClass }} font-semibold text-xs uppercase tracking-wide">
                                <a href="{{ $sortUrl }}"
                                   class="inline-flex items-center gap-1 hover:text-indigo-600 transition-colors group">
                                    ...
                                    @if($isActive)
                                            <span
                                                class="text-indigo-600 font-bold">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                                        @else
            ...
            @if($filter !== 'all')
                <div
                    class="rounded-2xl bg-white dark:bg-gray-800 border border-white/60 dark:border-gray-700 p-4 shadow-sm">
                    <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                        Filtered Total:
                        <span class="text-indigo-600 dark:text-indigo-400 ml-1">
                KES {{ number_format($filteredTotal, 0) }}
            </span>
                ...
                @if($transactions->hasPages())
                    <div class="flex items-center justify-center gap-3">
                        ...
                        @if($transactions->onFirstPage())
                            <span
                                class="px-4 py-2 text-sm font-medium text-gray-400 bg-gray-100 dark:bg-gray-700 dark:text-gray-600 rounded-lg cursor-not-allowed">
                    ← Prev
                </span>
                        @else
