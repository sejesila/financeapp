            ...
            <div class="bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6">
                ...
                <div class="flex items-center gap-3 mb-6 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                    ...
                    <p class="text-sm text-red-700 dark:text-red-300">
                        This will permanently remove {{ $entries->count() === 1 ? 'this interest entry' : 'all ' . $entries->count() . ' interest entries from this recording' }} and recalculate your account balance.
                    </p>
                </div>

                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                    {{ $entries->count() === 1 ? 'Transaction Details' : 'Entries in this Recording' }}
                </h3>

                @if($entries->count() === 1)
                    @php $entry = $entries->first(); @endphp
                    <dl class="space-y-3 mb-6">
                        ...
                        <div class="flex justify-between text-sm">
                            ...
                            <dd class="font-medium text-gray-900 dark:text-white">{{ $entry->date->format('M d, Y') }}</dd>
                        </div>
                        <div class="flex justify-between text-sm">
                            <dt class="text-gray-500 dark:text-gray-400">Description</dt>
                            <dd class="font-medium text-gray-900 dark:text-white">{{ $entry->description }}</dd>
                        </div>
                        <div class="flex justify-between text-sm border-t dark:border-gray-700 pt-3">
                            ...
                            <dd class="font-bold text-red-600 dark:text-red-400 text-base">
                                −KES {{ number_format($entry->amount, 0, '.', ',') }}
                            </dd>
                        </div>
                    </dl>
                @else
                    <div class="mb-4 max-h-56 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
                        @foreach($entries as $entry)
                            <div class="flex justify-between items-center px-4 py-2.5 text-sm">
                                <span class="text-gray-600 dark:text-gray-400">{{ $entry->date->format('M d, Y') }}</span>
                                <span class="font-medium text-gray-900 dark:text-white">KES {{ number_format($entry->amount, 2, '.', ',') }}</span>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <dl class="space-y-3 mb-6">
                        ...
                        <div class="flex justify-between text-sm">
                            ...
                            <dd class="font-medium text-gray-900 dark:text-white">
                                {{ $entries->first()->date->format('M d') }} – {{ $entries->last()->date->format('M d, Y') }}
                            </dd>
                        </div>
                        <div class="flex justify-between text-sm border-t dark:border-gray-700 pt-3">
                            <dt class="text-gray-500 dark:text-gray-400">Total Amount to Reverse</dt>
                            <dd class="font-bold text-red-600 dark:text-red-400 text-base">
                                −KES {{ number_format($entries->sum('amount'), 0, '.', ',') }}
                            </dd>
                        </div>
                    </dl>
                @endif
                ...
                <form method="POST"
                      action="{{ route('accounts.interest.reverse', ['account' => $account, 'batchId' => $batchId]) }}">
                    @csrf
                    @method('DELETE')
