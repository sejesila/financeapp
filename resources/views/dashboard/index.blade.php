...
<x-app-layout>
    <style>
        ...
        .balance-blur { filter: blur(8px); transition: filter 0.3s; }
    ...
    <div class="py-6 sm:py-12 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 min-h-screen">
        <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
            ...
            <div class="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <button id="toggleBalance" class="self-start sm:self-auto flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-xl shadow-md hover:shadow-lg transition-all duration-200">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
                ...
                @foreach($mainCards as $card)
                    <div class="stat-card rounded-2xl shadow-lg p-5 sm:p-6 text-white bg-gradient-to-br {{ $card['gradient'] }}">
                        ...
                        <div class="space-y-1">
                            <h3 class="text-3xl font-bold balance-amount">KES {{ number_format($card['amount'], 0, '.', ',') }}</h3>
                ...
                @foreach($quickStats as $stat)
                    <div class="stat-card bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">{{ $stat['label'] }}</span>
                    ...
                    <div class="stat-card bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 sm:p-6">
                        ...
                        <div class="space-y-4">
                            @foreach($card['rows'] as $row)
                                <div class="{{ isset($row['isBold']) ? 'pt-2 border-t border-gray-200 dark:border-gray-700' : '' }}">
                                    <div class="flex justify-between items-center {{ isset($row['progress']) ? 'mb-1' : '' }}">
                                        <span class="text-sm text-gray-600 dark:text-gray-400 {{ isset($row['isBold']) ? 'font-medium text-gray-700 dark:text-gray-300' : '' }}">
                                            {{ $row['label'] }}
                                        </span>
                                        <span class="text-{{ $row['color'] ?? 'gray' }}-600 dark:text-{{ $row['color'] ?? 'gray' }}-400 {{ isset($row['isBold']) || isset($row['plain']) ? 'text-lg font-bold' : 'text-sm font-semibold' }}">
                                    ...
                                    @if(isset($row['progress']))
                                        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                            <div class="bg-{{ $row['color'] }}-500 h-2 rounded-full transition-all duration-500" style="width: {{ $row['progress'] }}%"></div>
                                        </div>
                                    @endif
                                    @if(isset($row['percent']))
                                        <span class="inline-block mt-1 px-2 py-1 text-xs font-medium rounded-full bg-{{ $row['color'] }}-100 text-{{ $row['color'] }}-800">
            ...
            @if($accounts->count() > 0)
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-5 sm:p-6 mb-6 sm:mb-8">
                    <div class="flex justify-between items-center mb-5">
                        ...
                        <a href="{{ route('accounts.index') }}" class="text-sm text-blue-600 hover:text-blue-700 dark:text-blue-400 font-medium transition-colors">
                            Manage →
                        </a>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        @foreach($accounts as $account)
                            <div class="relative group">
                                <div class="bg-gradient-to-br {{ $accountGradients[$loop->index % count($accountGradients)] }} rounded-xl p-4 text-white shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    <div class="flex justify-between items-start mb-3">
                                        ...
                                        <div class="w-8 h-8 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $accountIcons[$account->type] ?? $accountIcons['default'] }}"/>
                                            </svg>
                                        </div>
                                    </div>
                                ...
                                <div class="hidden sm:flex mt-2 gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                    <a href="{{ route('accounts.show', $account) }}" class="flex-1 text-center px-2 py-2 bg-gray-100 dark:bg-gray-700 rounded-lg text-xs font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                                        View
                                    </a>
                                    <a href="{{ route('transactions.create', ['account_id' => $account->id]) }}" class="flex-1 text-center px-2 py-2 bg-blue-100 dark:bg-blue-900 rounded-lg text-xs font-medium text-blue-700 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors">
                                        Add Transaction
                                    </a>
                                </div>
                            </div>
                    ...
                    <div class="mt-5 pt-5 border-t border-gray-200 dark:border-gray-700">
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            @php
                                $accountStats = [
                                    ['label' => 'Total', 'value' => $accounts->sum('current_balance'), 'color' => 'gray'],
                                    ['label' => 'Highest', 'value' => $accounts->max('current_balance'), 'color' => 'green'],
                                    ['label' => 'Lowest', 'value' => $accounts->min('current_balance'), 'color' => 'orange'],
                                    ['label' => 'Average', 'value' => $accounts->avg('current_balance'), 'color' => 'blue']
                                ];
                            @endphp
                            ...
                            @foreach($accountStats as $stat)
                                <div class="text-center">
                                    ...
                                    <p class="text-base font-bold text-{{ $stat['color'] }}-600 dark:text-{{ $stat['color'] }}-400 balance-hidden hidden">***</p>
                                </div>
                            @endforeach
                        </div>
                    </div>
                            ...
                            @foreach($topExpenses->take(3) as $expense)
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="text-right flex-shrink-0">
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ round($percentage) }}%</p>
                                    </div>
                                </div>
                            @endforeach
                        </div>
