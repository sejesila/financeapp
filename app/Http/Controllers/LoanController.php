...
class LoanController extends Controller implements HasMiddleware
{
    ...
    public function index(Request $request)
    {
        ...
        $minYear = Loan::where('user_id', Auth::id())->min(DB::raw('YEAR(disbursed_date)')) ?? date('Y');
        $maxYear = date('Y');

        $activeLoans = Loan::with(['account', 'payments'])
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $dueDate = $loanType === 'other'
                ? $validated['due_date']
                : ($validated['due_date'] ?? $disbursedDate->copy()->addDays(30)->format('Y-m-d'));
            ...
            // M-Shwari deducts excise duty upfront
            if ($loanType === 'mshwari') {
            ...
            if ($loanType === 'other') {
                $message .= $customInterestAmount > 0
                    ? "Total repayment: KES " . number_format($totalRepayment, 0) . " (Principal: " . number_format($principalAmount, 0) . " + Interest: " . number_format($customInterestAmount, 0) . ")."
                    : "Total repayment: KES " . number_format($totalRepayment, 0) . " (No interest).";
                if ($dueDate) {
    ...
    public function show(Loan $loan)
    {
        ...
        $repayment     = $this->calculateTotalRepayment($loan->principal_amount, $loan->interest_rate, $loan->interest_amount);
    ...
    public function recordPayment(Request $request, Loan $loan)
    {
        ...
        try {
            ...
            if (($loanType === 'mshwari' || $loanType === 'kcb_mpesa') && $paymentAccount->id !== $loan->account_id) {
                ...
                throw new Exception("M-Shwari and KCB M-Pesa loans must be repaid from {$loanAccountName}.");
            }

            if ($paymentAccount->current_balance < $paymentAmount) {
                throw new Exception(
                    "Insufficient balance in {$paymentAccount->name}. " .
                    "Required: KES " . number_format($paymentAmount, 0) . ", " .
                    "Available: KES " . number_format($paymentAccount->current_balance, 0)
                );
            }
            ...
            if ($loan->balance <= 0) {
                ...
                if ($loanType === 'mshwari' && $daysElapsed <= 10) {
                    ...
                    Transaction::create([
                        ...
                        'category_id' => $this->firstOrCreateCategory('Loan Fees Refund', 'income')->id,
                        'type'        => 'income',
                    ...
                    Transaction::create([
                        'user_id'     => Auth::id(),
                        'account_id'  => $loan->account_id,
                        'category_id' => $this->firstOrCreateCategory('Facility Fee Refund', 'income')->id,
                        'type'        => 'income',
            ...
            if ($paymentAccount->id !== $loan->account_id) {
                Account::find($loan->account_id)?->updateBalance();
            }
            ...
            if ($earlyRepaymentCredit > 0) {
                $daysElapsed   = Carbon::parse($loan->disbursed_date)->diffInDays(Carbon::parse($paymentDate));
                $creditAccount = Account::find($loan->account_id)->name ?? 'your account';
                $successMessage .= $loanType === 'kcb_mpesa'
                    ? " Early repayment cashback of KES " . number_format($earlyRepaymentCredit, 0) . " (facility fee refunded) credited to {$creditAccount} for repaying within {$daysElapsed} days!"
                    : " Early repayment credit of KES " . number_format($earlyRepaymentCredit, 0) . " (20% of loan fees refunded) credited to {$creditAccount} for repaying within {$daysElapsed} days!";
            }

            return redirect()->route('loans.show', $loan->id)->with('success', $successMessage);
    ...
    public function destroy(Loan $loan)
    {
        ...
        try {
            ...
            DB::rollBack();
            Log::error('LoanController@destroy failed', ['loan_id' => $loan->id, 'error' => $e->getMessage()]);
    ...
    public function calculateBreakdown($principalAmount, $loanType = 'mshwari', $customInterestAmount = 0)
    {
        return match ($loanType) {
            'kcb_mpesa' => $this->calculateKcbMpesaBreakdown($principalAmount),
            'other'     => $this->calculateCustomBreakdown($principalAmount, $customInterestAmount),
            default     => $this->calculateMshwariBreakdown($principalAmount),
        };
    }
    ...
    public function calculateKcbMpesaBreakdown($principalAmount)
    {
        ...
        $total       = $principalAmount + $facilityFee + $interest;
        ...
        return [
            ...
            'early_repayment'         => round($principalAmount + $interest, 2),
    ...
    public function calculateCustomBreakdown($principalAmount, $interestAmount = 0)
    {
        return [
            ...
            'total_repayment' => round($principalAmount + $interestAmount, 2),
            'deposit_amount'  => $principalAmount,
        ];
    }
    ...
    public function calculateMshwariBreakdown($principalAmount)
    {
        ...
        $discount          = $totalLoanFees * $config['early_repayment_refund_rate'];
        ...
        return [
            ...
            'early_repayment'          => round($standardRepayment - $discount, 2),
            'early_repayment_discount' => round($discount, 2),
            'early_repayment_days'     => $config['early_repayment_days'],
        ];
    }
    ...
    private function getLoanTypes(): array
    {
        return [
            'mshwari' => [
                'name'                        => 'M-Shwari',
    ...
    private function calculateTotalRepayment($principalAmount, $interestRate = null, $interestAmount = null): array
    {
        if ($interestRate !== null && $interestRate > 0) {
            $interest = ($principalAmount * $interestRate) / 100;
            return ['interest' => round($interest, 2), 'total' => round($principalAmount + $interest, 2)];
        }

        if ($interestAmount !== null && $interestAmount > 0) {
            return ['interest' => round($interestAmount, 2), 'total' => round($principalAmount + $interestAmount, 2)];
        }

        return ['interest' => 0, 'total' => $principalAmount];
    }

    private function detectLoanType(string $source): string
    {
        $source = strtolower($source);

        if (str_contains($source, 'kcb')) {
            return 'kcb_mpesa';
        }

        if (str_contains($source, 'mshwari') || str_contains($source, 'm-shwari')) {
            return 'mshwari';
        }
    ...
    private function firstOrCreateCategory(string $name, string $type): Category
    {
        return Category::firstOrCreate(
            ['user_id' => Auth::id(), 'parent_id' => null, 'name' => $name],
