...
class AccountController extends Controller
{
     ...
     * Blocks if interest has already been recorded this calendar month.
     */
    public function recordInterestForm(Account $account)
    {
        ...
        if (!$this->interestService->canRecordTodayKey($account)) {
            return redirect()->route('accounts.show', $account)
                ->with('info', 'Interest has already been recorded for ' . now()->format('M Y') . '. Come back next month!');
        }
    ...
    /**
     * Store a monthly interest transaction for a savings account.
     */
    public function recordInterest(Request $request, Account $account)
    {
        ...
        if (!$this->interestService->canRecordTodayKey($account)) {
            return redirect()->route('accounts.show', $account)
                ->with('error', 'Interest has already been recorded for ' . now()->format('M Y') . '.');
        }
        ...
        $skippedMonthsCount = $this->interestService->getSkippedMonthsCount($account);

        if ($skippedMonthsCount !== null && $skippedMonthsCount > 0 && !$request->input('acknowledge_skipped')) {
            return redirect()->back()->withInput()
    ...
    public function reverseTransfer(Request $request, Account $account, Transfer $transfer)
    {
        ...
        // Delete any fee transactions tied to this transfer
        // $transfer->transactions()->delete(); // only if Transfer hasMany transactions; skip if not
